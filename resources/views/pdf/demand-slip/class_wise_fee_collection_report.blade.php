                ...
                <table>
                    ...
                    <tr>
                        <td>
                            ...
                            <h5>{{ __("(Total - ". ($classWiseFeeCollectionReport['total_report'] ?? "").")") }}</h5>
                        </td>
                    </tr>
                </table>
            </td>
    ...
    <table style="width: 100%; font-family: 'Inter', sans-serif; border-collapse: collapse; margin-top: 10px; border-left: 1px solid #EEEEEE; border-right: 1px solid #EEEEEE;">
        <tbody>
            <tr>
                ...
                <th style="margin:0;vertical-align: middle; text-align: center; padding: 5px; background: #EEEEEE;">{{ __('Total Collection: '. ($classWiseFeeCollectionReport['total_amount'] ?? 0)) }}</th>
            </tr>
            @if (!empty($classWiseFeeCollectionReport['reports']))

            @php
            $count = 0;
            @endphp

            @foreach ($classWiseFeeCollectionReport['reports'] as $report)
            @php
            $count++;
            @endphp
            <tr>
