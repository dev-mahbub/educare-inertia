...
<body>
    ...
    <div class="single-item" style="margin-bottom: 10px;">
        ...
        <table style="width: 100%; font-family: 'Inter', sans-serif; border-collapse: collapse; margin-bottom: 10px; margin-top: 10px;">
            <tbody>
                <tr>
                    ...
                    @if (!empty($studentReport['reports']['installment_wise_amounts']))
                    @foreach (array_keys($studentReport['reports']['installment_wise_amounts']) as $installment)
                ...
                @if (!empty($studentReport['reports']['head_wise_reports']))
                @foreach ($studentReport['reports']['head_wise_reports'] as $report)
                <tr>
                ...
                <tr>
                    ...
                    @if (!empty($studentReport['reports']['installment_wise_amounts']))
                    @foreach (array_keys($studentReport['reports']['installment_wise_amounts']) as $installment)
                    <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; border: 1px solid #ddd; color: #030105"><strong>{{ __($studentReport['reports']['installment_wise_amounts'][$installment] ?? 0) }}</strong></td>
                    @endforeach
                    @endif

                    <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; border: 1px solid #ddd; color: #030105">
                        <strong>
                            {{ __($studentReport['reports']['total_due'] ?? 0) }}
                        </strong>
                    </td>
                </tr>
            </tbody>
        ...
        <div class="bottom-content" style="width: 100%; text-align: left;">
            <div class="list" style="width: 100%; padding: 0;">
                <ul>
                    <li>
                        @if (!empty($studentReport['reports']))
                        {{ __("Total payable amount Rs.{$studentReport['reports']['total_due']} (Rs. {$studentReport['reports']['total_due_in_word']} )") }}
                        @else
