...
<head>
    <meta charset="UTF-8">
    <title>Yearly Head Wise Paid Summary Report </title>
</head>
...
<body>
        ...
        <tr>
            <td style="vertical-align: middle; text-align: center;">
                <table>
                    ...
                    <tr>
                        <td>
                            <h5>Yearly Head Wise Paid Summary</h5>
    ...
    <table style="width: 100%; font-family: 'Inter', sans-serif; border-collapse: collapse; margin-top: 10px; border-left: 1px solid #EEEEEE; border-right: 1px solid #EEEEEE;">
        <tbody>
            <tr>
                <th style="margin:0;vertical-align: middle; text-align: center; padding: 5px; background: #EEEEEE;">{{ __('Fee Type') }}</th>

                @if (!empty($yearlyHeadWiseSummaryReport['month_wise_amounts']))
                @foreach (array_keys($yearlyHeadWiseSummaryReport['month_wise_amounts']) as $headTitle)
                ...
                <th style="margin:0;vertical-align: middle; text-align: center; padding: 5px; background: #EEEEEE;">{{ __('Refund') }}</th>
                <th style="margin:0;vertical-align: middle; text-align: center; padding: 5px; background: #EEEEEE;">{{ __('Net Receipt') }}</th>
            </tr>
            @if (!empty($yearlyHeadWiseSummaryReport['reports']))
            @foreach ($yearlyHeadWiseSummaryReport['reports'] as $report)
            <tr>
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE; color: #030105">{{ __($report['fee_type'] ?? "") }}</td>

                @if (!empty($yearlyHeadWiseSummaryReport['month_wise_amounts']))
                @foreach (array_keys($yearlyHeadWiseSummaryReport['month_wise_amounts']) as $headTitle)
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE; color: #030105">
                    {{ __($report['month_wise_amounts'][$headTitle] ?? 0) }}
                </td>
                @endforeach
                @endif
                ...
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE; color: #030105">{{ __($report['refund'] ?? 0) }}</td>
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE; color: #030105">{{ __($report['net_receipt'] ?? 0) }}</td>
            </tr>
            @endforeach
            @endif
            ...
            <tr>
                ...
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; background: #CCCCCC; color: #030105">
                    {{ __($yearlyHeadWiseSummaryReport['month_wise_amounts'][$headTitle] ?? 0) }}
                </td>
                @endforeach
                @endif

                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; background: #CCCCCC; color: #030105">
                    {{ __($yearlyHeadWiseSummaryReport['total'] ?? 0) }}
                </td>
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; background: #CCCCCC; color: #030105">
                    {{ __($yearlyHeadWiseSummaryReport['refund'] ?? 0) }}
                </td>
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; background: #CCCCCC; color: #030105">
                    {{ __($yearlyHeadWiseSummaryReport['net_receipt'] ?? 0) }}
                </td>
            </tr>
        </tbody>
    </table>
