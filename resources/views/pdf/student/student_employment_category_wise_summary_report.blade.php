        ...
        <tr>
            <td style="vertical-align: middle; text-align: center;">
                <table>
                    ...
                    <tr>
                        <td>
                            <h4>{{ __("Student Report : ".($schoolData['academic_year'] ?? "")." / Employment Category") }}</h4>
                        </td>
                    </tr>
                </table>
            </td>
    ...
    <table style="width: 100%; font-family: 'Inter', sans-serif; border-collapse: collapse; margin-top: 10px; border-left: 1px solid #EEEEEE; border-right: 1px solid #EEEEEE;">
        <tbody>
            ...
            <tr>
                ...
                @foreach($categories as $category)
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE; color: #030105">
                    {{ __($report[$category] ?? 0) }}
                </td>
                @endforeach
                @endif
                <td style="margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE; color: #030105">
                    {{ __($report['total_count'] ?? 0) }}
                </td>
            </tr>
            @endforeach
            <tr>
                <td style="font-weight:bold; margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE;  background: #EEEEEE;">
                    {{ __("Total") }}
                </td>
                @if (!empty($categories))
                @foreach($categories as $category)
                <td style="font-weight:bold; margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE; background: #EEEEEE;">
                    {{ __($reports[$category] ?? 0) }}
                </td>
                @endforeach
                @endif
                <td style="font-weight:bold; margin: 0;vertical-align: top; text-align: center; padding:5px; border-bottom: 1px solid #EEEEEE;  background: #EEEEEE;">
                    {{ __($reports['total_count'] ?? 0) }}
                </td>
            </tr>
        </tbody>
    </table>
