...
const FeeMonthlyColumnCharts = ({ graphWeakerReport }) => {
  const chartOptions = {
    series: [{
      name: 'Student Subject Wise Report',
      data: graphWeakerReport?.map(item => item?.total_percentage),
    }],
    chart: {
      type: 'bar',
      height: 350,
    ...
    xaxis: {
      categories: graphWeakerReport?.map(item => [item?.first_name]),
      labels: {
        show: true,
        rotate: -445,
        rotateAlways: false,
