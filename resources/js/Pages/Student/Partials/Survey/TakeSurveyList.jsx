...
const TakeSurveyList = ({ survey }) => {
    ...
    // handle take survey save start
    const handleTakeSurveySave = (e) => {
        e.preventDefault();

        const hasValidationErorr = data?.response?.some(item => {
            return item?.answer == "" && item?.is_required_field == true;
        });

        if (!hasValidationErorr) {
            post(route('student_survey.take_save', survey?.id));
        }
    }
    // handle take survey save end
        ...
        <div className='bg-white/70 shadow-[0_2px_10px_0px_rgba(0,0,0,0.08)] p-[30px] pt-[25px] maxXs:p-[15px] rounded-lg mb-5'>
            ...
            <form onSubmit={dummyData}>
                ...
                <div className="flex flex-wrap gap-2.5 mt-5 justify-end">
                    <Link
                        href={route('student_survey.list')}
