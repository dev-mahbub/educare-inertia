...
const RegistrationAnalyticsChart = ({
    last7DaysRegistrationAnalysis
 }) => {
  const chartOptions = {
    series: [{
      name: 'Student Subject Wise Report',
      data: [...Object.values(last7DaysRegistrationAnalysis)?.map(item => item.total_registration)]
    ...
    xaxis: {
      categories: [
        ...Object.values(last7DaysRegistrationAnalysis)?.map(item => [item.registration_date])
      ],
      labels: {
        show: true,
        rotate: -445,
...
export default RegistrationAnalyticsChart;
