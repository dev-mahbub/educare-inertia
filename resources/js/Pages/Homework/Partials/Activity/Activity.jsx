...
const Activity = ({ assessment, students, assessmentTypes, comments }) => {
    ...
    const handleDetailStudent = (detailId) => {
        ...
        router.post(route('homework.activity', { id: assessment.id }), { current_student_id: detailId });
    }
    ...
    const handleCommentsStore = (e) => {
        e.preventDefault();
        post(route('homework.activity.comment.save'), data);
        reset()
    }
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route('homework.activity.delete', id));
            }
        });
    }
                    ...
                    <div className="bg-white/70 shadow-[0_2px_10px_0px_rgba(0,0,0,0.08)] p-[30px] pt-[27px] maxXs:p-[15px] rounded-lg mb-5">
                        ...
                        <div className='assesment-activity flex justify-between gap-0 sm:gap-5 flex-wrap'>
                            ...
                            <p>Created By: <span>{`${assessment?.user?.first_name ?? ''} ${assessment?.user?.middle_name ?? ''} ${assessment?.user?.last_name ?? ''}`}</span></p>
                                <p>Homework Type: <span> {assessment?.type}</span></p>
                            </div>
                            <div className='assesment-acitvity-info'>
                                ...
                                <tbody>
                                    {
                                        studentsData.length > 0 ? (
                                            studentsData.map((student, index) => <tr key={index} className={activeRow?.id === student.id ? 'bg-supportingA/40' : 'bg-transparent'}>
                                                ...
                                                <td>{student?.homework_student_assessment_comment?.assessment_status ?? 'InProgress'}</td>
                                                <td>{student?.homework_student_assessment_comment?.comment}</td>
                                                <td>
                                                    <button
                                                        className='text-info hover:underline'
                    ...
                    showEnterDetail && (
                        <div className='mt-[40px]'>
                            <div className="activity-chatbox">
                                <div className="chat-message oponent-message">
                                        ...
                                        <div className="activity-message-info">
                                            <div className='flex items-center gap-2 px-5'>
                                                <div className="flex items-center">
                                                    ...
                                                    <span>May 17, 11:54 PM</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="oponent-message-box relative">
