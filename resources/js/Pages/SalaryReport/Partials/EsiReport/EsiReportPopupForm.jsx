...
import EsiReportPopupInputForm from './EsiReportPopupInputForm';

export default function EsiReportPopupForm({
    className = '',
    listPopup,
    setListPopup,
    earningTypes,
    ...
    const handleGenerate = (e) => {
        ...
        if (data?.earning_type_ids?.length == 0) {
            toast.error("Please select earning for generate epf report!", {
                position: 'top-right',
                autoClose: 1500,
            });
        } else {
            ...
            router.post(route('salary_report.esi'), data);
        }
    }
    // handle generate end
