...
const HostelDeallocationReportTableList = ({
    classroomNames = [],
    studentDetails = [],
}) => {
    ...
    const handelSearch = (e) => {
        e.preventDefault();
        router.post(route('hostel_report.deallocation'), { classroom_id: data?.classroom_id });
        setLoading(false);
    }

    const handelReset = (e) => {
        e.preventDefault();
        router.get(route('hostel_report.deallocation'));
        setLoading(false);
    }
                ...
                <div className='flex flex-wrap gap-2.5 justify-between items-center mb-2.5'>
                    <div className="educare-card-title pb-none">
                        <h5>
                            ...
                            Hostel Deallocation Report
                        </h5>
                    </div>
                    <div className='flex flex-wrap gap-2.5 items-center'>
                        <div>
                            <span className='min-h-[30px] inline-block border px-4 leading-7 border-supportingA whitespace-nowrap rounded-2xl text-[14px] text-supportingA'>Total : {studentData?.length}</span>
                        </div>
                                        ...
                                        {studentData?.length > 0 ? (
                                            studentData?.map((item, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>{item?.deallocation_date_at}</td>
                                                    <td>{item?.note}</td>
                                                </tr>
                                            ))
                                        ) : (
...
export default HostelDeallocationReportTableList;
