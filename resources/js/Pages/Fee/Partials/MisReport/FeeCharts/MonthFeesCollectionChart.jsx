...
const MonthFeesCollectionChart = ({
  ...
  const chartOptions = {
    series: [{
        data: Object.values(monthWiseFeeCollection)?.map(amount => amount / 1000)
    }],
    chart: {
      type: 'bar',
      height: 350,
    ...
    xaxis: {
        categories: Object.keys(monthWiseFeeCollection)?.map(month => [month]),
      labels: {
        show: true,
        rotate: -445,
        rotateAlways: false,
    ...
    yaxis: {
      ...
      tickAmount: 5,
        // max: 100,
        max: Object.keys(monthWiseFeeCollection)?.length > 0 ? getMaxNumber(Math.max(
            ...Object.values(monthWiseFeeCollection)?.map(amount => formatNumber(amount / 1000))
        )) + 20 : 100,
      labels: {
        formatter: (val) => {
          return val / 1 + 'K'
...
export default MonthFeesCollectionChart;
