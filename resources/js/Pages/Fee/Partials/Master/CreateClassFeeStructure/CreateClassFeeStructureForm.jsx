...
const CreateClassFeeStructureForm = ({
    feeStructures = [],
    structureTypes = [],
    class_names = [],
    fees = [],
    ...
    const [checkedAllTypesBoolean, setCheckedAllTypesBoolean] = useState(false);
    ...
    const {
        ...
        title: "",
        description: "",
        structure_type: !hostelAvailable ? "Day Scholar" : "",
        ...
        old_fee_ids: selectedFeeIds,
    });

    useEffect(() => {
        if (Object.keys(errors)?.length <= 0) {
            setSelectedFeeIds(fees?.map(item => item?.id));
            setSelectedOldFeeIds(fees?.filter(item => !item?.is_admission_install)?.map(item => item?.id));
        }
    }, [fees]);
    ...
    useEffect(() => {
        ...
        setSelectedClassNames(class_names.filter(item => selectedClassIds.includes(item.id)));
    }, [selectedClassIds]);
    ...
    useEffect(() => {
        ...
        // setData('fee_type_amount_array', selectedFeeTypeAmountArray.filter(item => selectedFeeIds?.includes(item.fee_id)));
    ...
    useEffect(() => {
        ...
        // setData('old_fee_type_amount_array', selectedOldFeeTypeAmountArray.filter(item => selectedOldFeeIds?.includes(item.fee_id)));
    ...
    const receiveDataFromChild = (data) => {
        ...
        // setSelectedClassNames(class_names.filter(item => tempClassIds.includes(item.id)));
    ...
    // store selected fee id for new students end
    ...
    const handleResetForm = () => {
        ...
        setSelectedFeeTypes([])
        setSelectedFeeIds([])
        setSelectedFeeTypeAmountArray([]);
        setSelectedOldFeeIds([])
        setSelectedOldFeeTypeAmountArray([]);
        setFeeData({})
        setFormReset(true);
    }
    // handle reset form end

    // handle fee structure form submit start
    const handleClassFeeStructureData = (e) => {
        e.preventDefault();
        ...
        if (JSON.stringify(fee_ids.sort()) === JSON.stringify(selectedFeeIds.sort()) && JSON.stringify(old_fee_ids.sort()) === JSON.stringify(selectedOldFeeIds.sort())){
            post(route('fee.save_create_class_fee_structure'), {
                preserveScroll: true,
                onSuccess: ({ props }) => {
                    handleResetForm();
                },
    ...
    // handle fee structure form submit end
    ...
    // handle delete fee structure end
    ...
    return (
        <>
                ...
                <div className="xxxl:col-span-4 col-span-12">
                    <div className="educare-common-card">
                        <div className="bg-white/70 shadow-[0_2px_10px_0px_rgba(0,0,0,0.08)] p-[30px] pt-[27px] maxXs:p-[15px] rounded-lg mb-5">
                            ...
                            <form onSubmit={handleClassFeeStructureData}>
            ...
            <SelectClassPopup
                ...
                formReset={formReset}
                selectedClassNameIds={tempClassIds}
                setSelectedClassNameIds={setTempClassIds}
            />

            <SelectFeeTypePopup
                sendFeeTypeDataToParent={receiveFeeTypesDataFromChild}
                checkedAllTypesBoolean={checkedAllTypesBoolean}
                setCheckedAllTypesBoolean={setCheckedAllTypesBoolean}
...
export default CreateClassFeeStructureForm;
