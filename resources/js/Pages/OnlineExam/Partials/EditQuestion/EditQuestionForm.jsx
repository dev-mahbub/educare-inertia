...
const EditQuestionForm = ({
    ...
    // answer options
    const [answerOptions, setAnswerOptions] = useState(virtualQuestion?.answer_options ?? []);

    // selected asset
    const [selectedVirtualAsset, setSelectedVirtualAsset] = useState(virtualAssets?.find(item => item?.id == virtualQuestion?.virtual_asset_id) ?? {});
    ...
    const { data, setData, errors, put, reset, processing, recentlySuccessful } = useForm({
        class_name_id: virtualQuestion?.class_name_id ?? "",
        subject_id: virtualQuestion?.subject_id ?? "",
        language: virtualQuestion?.language ?? "English",
        question_type: virtualQuestion?.question_type ?? "",
        difficulty_level: virtualQuestion?.difficulty_level ?? "",
        online_topic_id: virtualQuestion?.online_topic_id ?? "",
        virtual_asset_id: virtualQuestion?.virtual_asset_id ?? "",
        question: virtualQuestion?.question ?? "",
        answer_options: answerOptions,
        answer_explanation: virtualQuestion?.answer_explanation ?? "",
        mark: virtualQuestion?.mark ? parseInt(virtualQuestion?.mark) : "",
        share_with: virtualQuestion?.share_with ?? "",
        is_published: virtualQuestion?.is_published ?? "",
    });
    ...
    // handle update question start
    const handleUpdateQuestion = (e) => {
        e.preventDefault();

        put(route('online_exam.update_question', virtualQuestion?.id), {
            onSuccess: () => {
                reset();
                setAnswerOptions([]);
                setSelectedVirtualAsset({});
