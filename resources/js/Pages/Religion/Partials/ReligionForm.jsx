...
import FollowUpPopupForm from "./ReligionEditPopupForm";
import Swal from 'sweetalert2';
import ReligionEditPopupForm from "./ReligionEditPopupForm";
import TextareaInput from "@/Components/TextareaInput";



export default function ReligionForm({ religions }) {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route('religion.destroy', id));
            }
        });
    }
    ...
    return (
        <>
                        ...
                        <div className="educare-classroom-table-wrapper">
                            <div className="educare-card-title">
                                <h5>
                                    ...
                                    Religion list <span>(Total : {religions?.length})</span>
                                </h5>
                            </div>
                            ...
                            <div className="educare-default-table xs:overflow-x-auto mb-[25px]">
                                <table>
                                    ...
                                    <tbody>
                                        {religions?.length > 0 ?
                                            religions?.map((item, indx) => (
                                                <tr key={item?.id}>
                                                    <td>{indx + 1}</td>
                                                    <td>{item?.name}</td>
                                                    <td>{item?.details.length > 50 ? item?.details.slice(0, 50) + '...' :  item?.details}</td>
                                                    <td>
                                            ...
                                            <tr>
                                                <td className="text-center text-red-500" colSpan="7">Religion not found</td>
                                            </tr>
                                        }
                                    </tbody>
                                </table>
            ...
            <ReligionEditPopupForm editPopupOpen={editPopupOpen} setEditPopupOpen={setEditPopupOpen} editData={editData}></ReligionEditPopupForm>
