...
const Activity = ({ assessment, students, assessmentTypes, comments }) => {
    ...
    const handleDetailStudent = (detailId) => {
        ...
        router.post(route('assessment.activity', { id: assessment.id }), { current_student_id: detailId });
    }
    ...
    const handleCommentsStore = (e) => {
        e.preventDefault();
        post(route('assessment.activity.comment.save'), data);
        reset()
    }
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route('assessment.activity.delete', id));
            }
        });
    }
            ...
            <div className="bg-white/70 shadow-[0_2px_10px_0px_rgba(0,0,0,0.08)] p-[30px] pt-[27px] maxXs:p-[15px] rounded-lg mb-5">
                    ...
                    <div className="bg-white/70 shadow-[0_2px_10px_0px_rgba(0,0,0,0.08)] p-[30px] pt-[27px] maxXs:p-[15px] rounded-lg mb-5">
                        ...
                        <div className='assesment-activity flex justify-between gap-0 sm:gap-5 flex-wrap'>
                            ...
                            <p>Created By: <span>{`${assessment?.user?.first_name ?? ''} ${assessment?.user?.middle_name ?? ''} ${assessment?.user?.last_name ?? ''}`}</span></p>
                                <p>Assesement Type: <span> {assessment?.type}</span></p>
                                <p>Total Marks: <span> {assessment?.max_mark}</span></p>
                                <p>Pass Marks: <span> {assessment?.pass_mark}</span></p>
                            </div>
                            <div className='assesment-acitvity-info'>
                                ...
                                <p>Duration: <span> {assessment?.duration}</span></p>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <tbody>
                                    {
                                        studentsData.length > 0 ? (
                                            studentsData.map((student, index) => <tr key={index} className={activeRow?.id === student.id ? 'bg-supportingA/40' : 'bg-transparent'}>
                                                ...
                                                <td>{student?.student_assessment_comment?.assessment_status ?? 'InProgress'}</td>
                                                <td>{student?.student_assessment_comment?.comment}</td>
                                                <td>{student?.student_assessment?.map((mark, index) => mark.mark)} </td>
                                                <td>
                                                    <button
                                                        className='text-info hover:underline'
                ...
                </div>
                {
                    (showEnterMarks || showEnterDetail) && (
                        <div>
                            <div className='mt-7'>
                                <div className="educare-common-card-title mb-2">
                                    <h5>
                                        Enter marks for {`${activeRow?.first_name ?? ''} ${activeRow?.middle_name ?? ''} ${activeRow?.last_name ?? ''}`} and admission number is {activeRow?.admission_no ?? ''}
                                    </h5>
                                </div>
                                <form onSubmit={handleMarkStore}>
                                    <div className='flex flex-wrap items-center gap-2'>
                                        <div className="educare-input-field-styles max-w-[80px]">
                                            <TextInput
                                                value={
                                                    data.enter_mark
                                                }
                                        ...
                                        <div>
                                            <p className='text-headingLight font-semibold'>Out of 100</p>
                                        </div>
                                        <div>
                                            <PrimaryButton
                                                // disabled={processing}
