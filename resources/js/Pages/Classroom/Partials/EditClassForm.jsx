...
const EditClassForm = ({
    ...
    const [startDate, setStartDate] = useState(
        new Date(classrooms?.start_date_at)
    );
    const [repeatTillDate, setRepeatTillDate] = useState(
        new Date(classrooms?.end_date_at)
    );

    const intStartTime = moment(classrooms?.start_time_at, "HH:mm:ss").toDate();
    const intEndTime = moment(classrooms?.end_time_at, "HH:mm:ss").toDate();

    const [startTime, setStartTime] = useState(new Date(intStartTime));
    const [endTime, setEndTime] = useState(new Date(intEndTime));

    const [classDays, setClassDays] = useState(
        classrooms?.class_day.split(", ")
    );
    ...
    const { data, setData, errors, put, reset, processing } = useForm({
        subject_id: classrooms?.subject_id,
        class_name_id: classrooms?.class_name_id,
        start_date_at: classrooms?.start_date_at,
        end_date_at: classrooms?.end_date_at,
        start_time_at: classrooms?.start_time_at,
        end_time_at: classrooms?.end_time_at,
        class_notes: classrooms?.class_notes,
        classDays: classrooms?.class_day,
    ...
    const handleClassUpdate = (e) => {
        ...
        put(route("classroom.time_table_update", classrooms.id), data, {
            preserveScroll: true,
            onSuccess: () => {
                reset();
