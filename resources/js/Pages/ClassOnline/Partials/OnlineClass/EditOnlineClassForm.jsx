...
const EditOnlineClassForm = ({
    ...
    const [repeatableDays, setRepeatableDays] = useState(online_class?.repeatable_days ?? []);

    const intStartTime = moment(online_class?.start_time, "HH:mm:ss").toDate();
    const intEndTime = moment(online_class?.end_time, "HH:mm:ss").toDate();
    ...
    const { data, setData, errors, put, reset, processing } = useForm({
        classroom_id: online_class?.classroom_id ?? "",
        subject_id: online_class?.subject_id ?? "",
        start_date: online_class?.start_date ? new Date(online_class?.start_date) : "",
        start_time: startTime,
        end_time: endTime,
        live_class_url: online_class?.live_class_url ?? "",
        notes: online_class?.notes ?? "",
        repeat_date: online_class?.repeat_date ? new Date(online_class?.repeat_date) : "",
        repeatable_days: online_class?.repeatable_days ?? []
    });
    ...
    // handle update online class start
    ...
    // handle update online class end
    ...
    const handleClassroomChange = (e) => {
        ...
        router.post(route('online_class.edit', online_class?.id), form_data);
    }
