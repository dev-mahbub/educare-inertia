...
const TeacherCredentialList = ({
    ...
    useEffect(() => {
        setTeachersData(teachers?.filter(item => item?.title?.toLowerCase()?.includes(data?.search?.toLowerCase()?.trim())));
    ...
    const handleTeacherSelect = (name, value, id) => {
        ...
        if (name === "select_all") {
            ...
            if (value) {
                newCheckedIds = teachers.map(item => item.id);
            } else {
                newCheckedIds = [];
            }
            ...
            newFormData.selected_ids = newCheckedIds;
            newFormData.select_all = newCheckedIds.length === teachers.length;
        }

        const newCheckedData = teachers.filter(item => newCheckedIds.includes(item.id));
                ...
                <div className="bg-white/70 shadow-[0_2px_10px_0px_rgba(0,0,0,0.08)] p-[30px] pt-[27px] maxXs:p-[15px] rounded-lg mb-5">
                    <div className="grid grid-cols-12 gap-5">
                        ...
                        <div className="col-span-12 lg:col-span-6">
                            ...
                            <div>
                                <span className='text-headingLight font-medium'>{checkedData.length} Teacher selected</span>
                            </div>
                            <div className="educare-default-table xs:overflow-x-auto table-body-dark">
                                <table>
                                    <tbody>
