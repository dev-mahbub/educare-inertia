...
const PaidDiscountReportList = ({
    ...
    const [enqInnerActive, setEnqInnerActive] = useState(Array(Object.keys(paidDiscountReportData)?.length).fill(false))
    ...
    useEffect(() => {
        setTotalAmount(() => {
            ...
            for (const key in paidDiscountReportData) {
                if (Object.hasOwnProperty.call(paidDiscountReportData, key)) {
                    total_amount += paidDiscountReportData[key]?.total_discount
                }
            }
        ...
        setEnqInnerActive(Array(Object.keys(paidDiscountReportData)?.length).fill(false));
    }, [paidDiscountReportData]);
                    ...
                    <div className="educare-admission-list-inner-wrapper">
                        <div className="educare-admission-list">
                            <table>
                                ...
                                :
                                    <tbody>
                                        {/*dynamic discount going to display here*/}
                                        {Object.keys(paidDiscountReportData)?.length > 0 ? (
                                            <>
                                            {Object.values(paidDiscountReportData)?.map?.((discountItem, index) => (
                                                <>
                                                    <tr>
                                                        <td colSpan={7}>
                                                            <strong className='text-[15px] block max-w-[220px]'>{discountItem?.discount?.title ?? 'Unknown Discount'}
                                                                <button
                                                                    type="button"
                                                                    ...
                                                                    {Object.keys(discountItem?.data)?.length > 0 ?
                                                                        Object.values(discountItem?.data)?.map((item, index) => (
                                                                            <tr key={index}>
                                                                                ...
                                                                                <td>{discountItem?.discount?.title ?? 'Unknown Discount'}</td>
...
export default PaidDiscountReportList;
