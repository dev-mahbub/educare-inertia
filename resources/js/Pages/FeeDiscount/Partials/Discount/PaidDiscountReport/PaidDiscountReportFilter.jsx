...
const PaidDiscountReportFilter = ({
    fees = [],
    discounts = [],
    setLoading,
    discountPaidReport = [],
    paidDiscountReportData = [],
    setPaidDiscountReportData
}) => {
    ...
    // handle paid disocunt report filter start
    const getPaidStudentFeeDiscounts = (e) => {
        e.preventDefault();
        ...
        if (form_data.from_fee_id != null && form_data.to_fee_id != null) {
            setLoading(false);

            router.post(route('fee_discount.paid_report'), form_data)
        }
        else {
    ...
    // handle paid disocunt report filter end
    ...
    const handleSearch = (e) => {
        ...
        if (Object.keys(discountPaidReport)?.length > 0) {
            const filteredPaidDiscountReports = Object.values(discountPaidReport)?.map((discountItem) => {
                const filteredData = Object.values(discountItem?.data)?.filter(item => {
                    ...
                    const motherName = (`${item?.student?.mother?.first_name} ${item?.student?.mother?.middle_name} ${item?.student?.mother?.last_name}`)?.toLowerCase();
                    const discountTitle = item?.discount?.title?.toLowerCase();
                    const discountAmount = item?.total_fee_discount?.toString()?.toLowerCase();
                    ...
                    return (
                        ...
                        motherName && motherName?.includes(inputText) ||
                        discountTitle && discountTitle?.includes(inputText) ||
                        discountAmount && discountAmount?.includes(inputText)
                    )
                });
            ...
            setPaidDiscountReportData(filteredPaidDiscountReports);
        }
        else {
            setPaidDiscountReportData([]);
        }
            ...
            <div className='educare-admission-filtar-bar-area z-[4] relative'>
                <div className="py-3 pt-0 educare-header-filtar-bar-wrap">
                    <div className="educare-header-filtar-bar-main">
                        <form onSubmit={PaidDiscountData}>
                                ...
                                <div className="educare-header-filtar-bar-action educare-filter-action-btn">
                                    <div>
                                        ...
                                        >
                                            <button
                                                ...
                                                onClick={getPaidStudentFeeDiscounts}
                                            >
                                                <i className="icon-search-interface-symbol"></i>
                                            </button>
                                        </Tooltip>
                                    </div>
                                    {Object.keys(paidDiscountReportData)?.length > 0 &&
                                        <div>
                                            <Tooltip
                                                title="PDF"
                                                placement="top"
                                    ...
                                    {Object.keys(paidDiscountReportData)?.length > 0 &&
                                        <div>
                                            ...
                                            >
                                                <a
                                                    href={route('export_excel.paid_discount_report', params)}
                                                    target="_blank"
                                    ...
                                    <div>
                                        ...
                                        >
                                            <Link
                                                href={route('fee_discount.paid_report')}
...
export default PaidDiscountReportFilter;
