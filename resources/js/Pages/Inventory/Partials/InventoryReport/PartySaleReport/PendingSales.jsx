...
const PendingSales = ({
    ...
    const [pendingSaleData, setPendingSaleData] = useState(pendingSale);
    const [enqInnerActive, setEnqInnerActive] = useState('');

    const amountTotal = pendingSaleData?.reduce((acc, curr) => acc + parseFloat(curr?.total || 0), 0);
    const paidTotal = pendingSaleData?.reduce((acc, curr) => acc + parseFloat(curr?.paid_amount || 0), 0);
    const dueTotal = pendingSaleData?.reduce((acc, curr) => acc + parseFloat(curr?.due_amount || 0), 0);

    useEffect(() => {
        setPendingSaleData(pendingSale)
        setLoading(false);
    }, [pendingSale]);
                ...
                <div className="educare-header-filtar-bar-action educare-filter-action-btn">
                    {pendingSale?.length > 0 &&
                        <div>
                            ...
                            >
                                <a
                                    href={route('pdf_account.party_wise_sale_report', {sale_type: 'pending', ...params})}
                                    target='_blank'
                            ...
                            <div className="educare-admission-list pb-none">
                                <table>
                                    ...
                                    {loading ? (
                                        ...
                                        <tbody>
                                            {pendingSaleData?.length > 0 ? (
                                                pendingSaleData?.map((item, index) => (
                                                    <React.Fragment>
                                                        <tr key={index}>
                                                            <td>
                                                                {++index}
