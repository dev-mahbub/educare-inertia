...
export default function CancelSaleReturnPopup({
    cancelSaleReturnPopup,
    setCancelSaleReturnPopup,
    saleLedgerReturnId,
    setSaleLedgerReturnId,
    formData
 }) {
    const {
        data,
    ...
    useEffect(() => {
        setData((prevData) => ({
            ...prevData,
            sale_ledger_return_id: saleLedgerReturnId
        }));
    }, [saleLedgerReturnId]);
    ...
    const handleCancelSaleReturnSuccess = () => {
        ...
        router.post(route('sale_return_report.list'), form_data);
    }
    ...
    const handleCancelSaleReturn = (e) => {
        ...
        if(data?.cancel_reason != "") {
            patch(route("sale_return.cancel", saleLedgerReturnId), {
                preserveScroll: true,
    ...
    const closeModal = () => {
        setCancelSaleReturnPopup(false);
        setSaleLedgerReturnId(null);
        reset();
    };
    ...
    return (
        <section className={`educare-admission-follow-up-area space-y-6`}>
            <Modal show={cancelSaleReturnPopup} onClose={closeModal}>
