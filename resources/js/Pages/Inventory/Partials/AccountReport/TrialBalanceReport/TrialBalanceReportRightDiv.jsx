...
const TrialBalanceReportRightDiv = ({
    ...
    const filteredLedgerSummary = useMemo(() => {
        ...
        return selectedAccountGroup?.ledgers?.filter((item) => {
    ...
    }, [selectedAccountGroup, data?.search]);
        ...
        <>
            <div className="educare-card-title mr-auto pb-none mb-2.5">
                <h5>
                    ...
                    {selectedAccountGroup?.title} Summary
                </h5>
            </div>
                ...
                <div className="flex flex-wrap gap-2.5 justify-between items-center mb-2.5">
                    <div className="educare-card-title pb-none">
                        <span className="min-h-[30px] inline-block border px-4 leading-7 border-supportingA whitespace-nowrap rounded-2xl text-[14px] text-supportingA">
                            Total : {selectedAccountGroup?.ledgers?.length ?? 0}
                        </span>
                    </div>
            ...
            <div className="educare-default-table xs:overflow-x-auto">
                <table>
                    ...
                    <tbody>
                        {selectedAccountGroup?.id != null && selectedAccountGroup?.ledgers?.length > 0 ?
                            <>
                                {filteredLedgerSummary?.length > 0 ?
                                    filteredLedgerSummary?.map((item, index) => (
                                        <tr key={index}>
                                            <td>{item?.title}</td>
                                ...
                                <tr>
                                    <td>Total</td>
                                    <td>{selectedAccountGroup?.ledgers?.reduce((total, item) => total + parseFloat(item.debit ?? 0), 0)?.toFixed(2)}</td>
                                    <td>{selectedAccountGroup?.ledgers?.reduce((total, item) => total + parseFloat(item.credit ?? 0), 0)?.toFixed(2)}</td>
                                </tr>
                            </>
                        :
                            <tr>
...
export default TrialBalanceReportRightDiv;
