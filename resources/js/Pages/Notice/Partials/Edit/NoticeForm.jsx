...
const NoticeForm = ({
    ...
    const [startDate, setStartDate] = useState(notice?.start_date ? new Date(notice?.start_date) : null);
    const [endDate, setEndDate] = useState(notice?.end_date ? new Date(notice?.end_date) : null);
    const [classroomIds, setClassroomIds] = useState(notice?.notice_classrooms?.map(item => item?.classroom_id));
    ...
    // handle notice save start
    const handleNoticeSave = (e) => {
        e.preventDefault();
