...
class HomeworkApiController extends ControllerApi
{
    private $_upload;
    ...
    /**
     * @OA\Get(
     *    path="/homeworks/show/{id}",
     *    operationId="showHomework",
     *    tags={"Homework"},
     ...
     *  )
     */
    public function showHomework(Request $request, int $id)
    {
    ...
    /**
     * @OA\Get(
     *    path="/homeworks/student/submission",
    ...
    /**
     * @OA\Post(
     * path="/homeworks/students/status/change",
    ...
    /**
     * @OA\Post(
     * path="/homeworks/student/submit",
     * summary="Submit Homework",
     * description="Submit Homework",
     * operationId="submitStudentHomework",
     * tags={"Homework"},
     * @OA\RequestBody(
     *    required=true,
     *    description="Submit Student Homework",
     *    @OA\JsonContent(
    ...
    *         ),
     ...
     * )
     */
    public function submitStudentHomework(Request $request)
    {
        $setting = SchoolSetting::where('status', Status::ACTIVE)
            ->where('school_id', $request->schoolId)
            ->first();
        ...
        if (!empty($request->file('ass_file'))) {
            $ass_file = $this->_upload->uploadImage($request, 'ass_file', 'homework_image');
        }
        ...
        if (!empty($request->comment)) {
            $dataArray = [
                ...
                'ass_file' => $ass_file,
                'assessment_status' => isset($request->status) ? $request->status : HomeworkStatus::INPROGRESS->value,
                'commented_by' => $request->commentedBy
            ];

            $homework =  $this->homeworkRepository->createHomeworkStudentActivityComment($dataArray);
     ...
     * )
     */
    public function deleteHomework(Request $request, int $id)
    {
        if ( !empty($request->schoolId) ) {
    ...
    /**
     * @OA\Get(
     *    path="/homeworks/student/view",
     *    operationId="viewStudentHomework",
    ...
    *       @OA\Property(property="schoolId", type="integer", example="1", description="school Id"),
    ...
    public function viewStudentHomework(Request $request)
    {
        if (!empty($request->classroomId)) {
            $homeworks = $this->homeworkRepository->getHomeworksByClassroomIdWithFilters(
            $request->classroomId,
    ...
    *       @OA\Property(property="schoolId", type="integer", example="1", description="school Id"),
    ...
    {
        if (!empty($request->studentId) && !empty($request->homeworkId)) {
            $academicYearId = getAcademicYearIdFromSchoolId($request->schoolId);
            $comments = $this->homeworkRepository->getHomeworkStudentAssessmentComments(
                $request->schoolId ?? null,
                $academicYearId,
                $request->homeworkId ?? null,
    ...
    /**
     * @OA\Delete(
     *     path="/homeworks/student/comments/delete/{}",
     *     tags={"Homework"},
     *     summary="Delete Homework Comments",
     *     operationId="deleteHomeworkComments",
     *     @OA\Parameter(
     *         name="id",
     *         in="path",
     *         description="id to delete",
     ...
     * )
     */
    public function deleteHomeworkComments(Request $request, int $id)
    {
        if (!empty($request->schoolId)) {
            try {
                ...
                if (!$homeworkActivityComment) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Homework comment not found or does not belong to this school.',
                    ], 404);
                }
        
                $homeworkActivityComment->delete();
                return response()->json([
                    'success' => true,
                    'message' => 'Homework Comment deleted successfully.',
                    'data' => $homeworkActivityComment
                ], 200);
        
            } catch (\Exception $e) {
                return response()->json([
                    'success' => false,
                    'message' => 'Homework comment not found or already deleted.',
                ], 404);
            }
        } else {
            return response()->json([
