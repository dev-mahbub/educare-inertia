...
class VoucherReportExport implements FromCollection, WithEvents
{
        ...
        if (!empty($this->voucherReports['reports'])) {
            ...
            foreach ($this->headings as $index => $heading) {
                if ($index == 0) {
                    ...
                    $rows[$rows_count][] = $this->voucherReports['total_amount'] ?? 0;
                } else if ($heading == 'total_paid') {
                    $rows[$rows_count][] = $this->voucherReports['total_paid'] ?? 0;
                } else if ($heading == 'total_due') {
                    $rows[$rows_count][] = $this->voucherReports['total_due'] ?? 0;
                } else {
                    $rows[$rows_count][] = "";
                }
            }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A1', 'Student Voucher List');
