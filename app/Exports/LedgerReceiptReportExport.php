...
class LedgerReceiptReportExport implements FromCollection, WithEvents
{
    protected $receiptReport = [];
    protected $receiptSummary = [];
    ...
    public function __construct(array $receiptReport, array $receiptSummary, string $schoolTitle, string $academicYear, string $reportDateTitle)
    {
    ...
    public function collection()
    {
        ...
        // receipt report
        if (!empty($this->receiptReport['reports'])) {
            foreach ($this->receiptReport['reports'] as $report) {
            ...
            foreach ($this->headings as $index => $heading) {
                if ($heading == 'narration') {
                    ...
                    $rows[$rows_count][] = ($this->receiptReport['total_amount'] ?? 0) > 0 ? $this->receiptReport['total_amount'] : "0";
                } else {
                    $rows[$rows_count][] = "";
                }
            }
        ...
        foreach ($this->headings as $index => $heading) {
            ...
            if ($heading == 'receipt_date') {
                $rows[$rows_count + 2][] = 'Receipt Summary';
            } else {
                $rows[$rows_count + 2][] = "";
            }
        ...
        if (!empty($this->receiptSummary['reports'])) {
            $count = 0;
            $rows_count = count($rows) + 1;

            foreach ($this->receiptSummary['reports'] as $report) {
                foreach ($this->headings as $heading) {
                    if ($heading == 'amount') {
            ...
            foreach ($this->headings as $index => $heading) {
                if ($heading == 'narration') {
                    ...
                    $rows[$rows_count][] = ($this->receiptSummary['total_amount'] ?? 0) > 0 ? $this->receiptSummary['total_amount'] : "0";
                } else {
                    $rows[$rows_count][] = "";
                }
            }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Set heading for merged cells
                $event->sheet->setCellValue('A2', "Inventory Receipt Report");
