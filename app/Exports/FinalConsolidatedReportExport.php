...
class FinalConsolidatedReportExport implements FromCollection, WithEvents
{
        ...
        if (!empty($this->consolidatedReport['reports'])) {
            ...
            foreach ($this->consolidatedReport['reports'] as $report) {
                foreach ($this->headings as $heading) {
                    if (in_array($heading, ['admission_no', 'roll_no', 'student_name'])) {
                        $rows[$count][] = $report['student'][$heading] ?? "";
                    } else if ($heading == 'total') {
                        $rows[$count][] = $report['total_mark'] ?? "";
                    } else {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A1', "Final Consolidated Report For Class - " . $this->classroomTitle . "(" . $this->academicYear . ")");
